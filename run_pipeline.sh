...
usage() {
    ...
    echo "  --use-neo4j                Use Neo4j for graph storage"
...
if [ -z "$MODEL" ]; then
    if [ "$PROVIDER" = "glm" ] && [ ! -z "$GLM_MODEL" ]; then
        MODEL="$GLM_MODEL"
...
# Print configuration
echo "[PIPELINE] Starting pipeline integration..."
echo "[PIPELINE] Ingest directory: $INGEST_DIR"
echo "[PIPELINE] NER directory: $NER_DIR"
echo "[PIPELINE] Chunker directory: $CHUNKER_DIR"
echo "[PIPELINE] Provider: $PROVIDER"
echo "[PIPELINE] Model: $MODEL"

# Build the command
CMD="python pipeline_integration.py --ingest-dir \"$INGEST_DIR\" --ner-dir \"$NER_DIR\" --chunker-dir \"$CHUNKER_DIR\" --config \"$CONFIG\""
...
# Run the command
echo "[PIPELINE] Running pipeline integration command..."
echo $CMD
eval $CMD
